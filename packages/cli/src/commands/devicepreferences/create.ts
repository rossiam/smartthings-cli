...
export default class DevicePreferencesCreateCommand extends APICommand {
	...
	async getInputFromUser(): Promise<DevicePreferenceCreate> {
		const name = await askForString('Preference name:',
			input => !input || input.match(/^[a-z][a-zA-Z0-9]{2,23}$/) ? true : 'must be camel case starting with a lowercase letter and 3-24 characters')
			...
			const defaultValue = await askForString('Optional default value.',
				input => {
					if (minLength !== undefined && input.length < minLength) {
						return `default must be at least minLength (${minLength}) characters`
					}
		...
		if (preferenceType === 'enumeration') {
			...
			do {
				name = await askForString('Enter a name for the next option, if any.')
				if (name) {
