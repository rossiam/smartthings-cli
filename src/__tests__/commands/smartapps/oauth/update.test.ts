...
import type { CommandArgs } from '../../../../commands/smartapps/oauth/update.js'
...
const corsDomainsDefMock = buildInputDefMock<string>('CORS Domains Mock')
...
const { default: cmd } = await import('../../../../commands/smartapps/oauth/update.js')
...
describe('handler', () => {
	...
	const apiAppsUpdateOauthMock = jest.fn<typeof SmartAppsEndpoint.prototype.updateOauth>()
	const client = {
		smartapps: {
	...
	it('prompts user when no app specified', async () => {
		...
		const updatedOAuthData = { clientName: 'client-name' } as SmartAppOAuthResponse
		apiAppsUpdateOauthMock.mockResolvedValueOnce(updatedOAuthData)
		const updateRequest = { clientName: 'client-name' } as SmartAppOAuthRequest
	...
	const userGeneratedRequest = { clientName: 'client-name' } as SmartAppOAuthRequest
