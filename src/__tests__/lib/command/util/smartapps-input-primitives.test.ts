...
const corsDomainDefMock = buildInputDefMock<string>('CORS Domain Mock')
stringDefMock.mockReturnValueOnce(corsDomainDefMock)
const corsDomainsDefMock = buildInputDefMock<string[]>('CORS Domains Mock')
arrayDefMock.mockReturnValueOnce(corsDomainsDefMock)

const {
	corsDomainsDef,
	oauthAppScopeDef,
	redirectUrisDef,
} = await import('../../../../lib/command/util/smartapps-input-primitives.js')


expect(oauthAppScopeDef).toBe(oauthAppScopeDefMock)
expect(redirectUrisDef).toBe(redirectUrisDefMock)
expect(arrayDefMock).toHaveBeenCalledWith(
	'Redirect URIs',
	redirectUriDefMock,
	expect.objectContaining({ minItems: 0 }),
)
expect(corsDomainsDef).toBe(corsDomainsDefMock)
expect(arrayDefMock).toHaveBeenCalledWith(
	'CORS Domains',
	corsDomainDefMock,
	expect.objectContaining({ minItems: 0 }),
